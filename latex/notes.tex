...
\begin{example} Пусть $J$ - конечное множество индексов, $A:J\to\mathbb{R}^{2}_{++}$ тогда функция\\
    $h(p):=\min\limits_{j\in J}\set{A^{1}_{j}p^{1} + A^{2}_{j}p^{2}}$
    является строго положительной неоклассической функцией.
\end{example}


\begin{theorem} Пусть $h\in\hat{\Phi}_{2}$, $U\subset\mathbb{R}^{2}_{++}$ -- открытое непустое множество. Тогда существует непустое открытое подмножество $V\subset U$ что $\forall p\in V$ кривые $h(x)=1$ и $h(p\circ x) = 1$ имеют конечное кол-во пересечений, причем все эти пересечения происходят трансверсально. 
\end{theorem}
