...
\begin{proposition}\label{prop:transversal} Пусть $h,g\in\hat{\Phi}_{2},x_{0}\in\Rpp:h(\hat{x})=g(\hat{x})=1$. Линии уровня $h(x)=1$ и $g(x)=1$ пересекаются в $x_{0}$ трансверсально, если и только если $\partial h(x_{0})\cap\partial g(x_{0}) = \varnothing.$
Более того, точка $x_{0}\in\partial\Rpp$ не может быть трансверсальным пересечением этих кривых.
\end{proposition}

\begin{theorem}\label{thm:finite} Пусть $h\in\hat{\Phi}_{2}$, $U\subset\Rpp$ -- открытое непустое множество. Тогда существует непустое открытое подмножество $V\subset U$ что $\forall p\in V$ кривые $h(x)=1$ и $h(p\circ x) = 1$ имеют конечное кол-во пересечений, причем все эти пересечения происходят трансверсально.
\end{theorem}
...
% Приложение B: \section{Доказательство теоремы~\ref{thm:finite}}
